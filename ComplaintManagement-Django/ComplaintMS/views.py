...
def counter(request):
        ...
        dataset=Complaint.objects.values('Type_of_issue').annotate(total=Count('status'),solved=Count('status', filter=Q(status='1')),
                  notsolved=Count('status', filter=Q(status='3')),inprogress=Count('status',filter=Q(status='2'))).order_by('Type_of_issue')
...
def complaints(request):
    ...
    if request.method == 'POST':
        ...
        #        send_mail('Hi Complaint has been Received', 'Thank you for letting us know of your concern, Have a Cookie while we explore into this matter.  Dont Reply to this mail', 'testerpython13@gmail.com', [mail],fail_silently=False)
               ...
               messages.add_message(request,messages.SUCCESS, f'Your issue has been registered!')
...
def allcomplaints(request):
        ...
        if drop:
                c=c.filter(Q(Type_of_issue__icontains=drop))
        if comp:
                c=c.filter(Q(Type_of_isssue__icontains=comp)|Q(Description__icontains=comp)|Q(Subject__icontains=comp))
        if request.method=='POST':
                ...
                if forms.is_valid():
                        ...
                        obj.save()
                        messages.add_message(request,messages.SUCCESS, f'The issue has been updated!')
...
def solved(request):
        ...
        if comp:
                ...
                c=c.filter(Q(Type_of_issue__icontains=comp)|Q(Description__icontains=comp)|Q(Subject__icontains=comp))
        if request.method=='POST':
                cid=request.POST.get('cid2')
                print(cid)
                project = Complaint.objects.get(id=cid)
...
def pdf_viewer(request):
    ...
    '''Class = Complaint.objects.filter(id=cid).values('Class')'''
    Subject = Complaint.objects.filter(id=cid).values('Subject')
    Type = Complaint.objects.filter(id=cid).values('Type_of_issue')
    ...
    '''for val in Class:
            detailclass=("Class: {}".format(val['Class']))'''
    for val in Subject:
            detailsubject=("{}".format(val['Subject']))
    for val in Type:
            detailtype=("{}".format(val['Type_of_issue']))
    ...
    if detailsubject == '1':
          detailsubject = "Issued To : Teaching Staff"
    if detailsubject == '2':
          detailsubject = "Issued To: HeadMaster"
    if detailsubject == '3':
          detailsubject = "Issued To: District Educational Officer (DEO)"
    if detailsubject == '4':
          detailsubject = "Issued To: Chief Educational Officer (CEO)"    
    if detailsubject == '5':
          detailsubject = "Issued To: Other Educational Officers"

    if detailtype=='1':
            detailtype="Type of Issue: Play Ground"
    if detailtype=='3':
            detailtype="Type of Issue: Water Supply"
    if detailtype=='2':
            detailtype="Type of Issue: WashRoom"
    if detailtype=='4':
            detailtype="Type of Issue: Food Supply"
    if detailtype=='5':
            detailtype="Type of Issue: Sports & Games"
    if detailtype=='6':
            detailtype="Type of Issue: Technical Equipment"
    if detailtype=='7':
            detailtype="Type of Issue: Other Issues"

    p.drawString(25, 770,"Report:")
    p.drawString(30, 750,detailname)
    ''' p.drawString(30, 730,detailclass)'''
...
def pdf_view(request):
    ...
    if detailtype=='1':
            detailtype="Type of Issue: PlayGround"
    if detailtype=='3':
            detailtype="Type of Issue: Water Supply"
    if detailtype=='2':
            detailtype="Type of Issue: WashRoom"
